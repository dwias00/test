(() => {
  ...
  const SESSION_KEY = "cbt_smp_remote_session_v3";
  const CONFIG_KEY = "cbt_smp_remote_config_v3";
  const ATTEMPT_BACKUP_KEY = "cbt_smp_attempt_backup_v3";
  ...
  function applyBranding(settings = state.settings) {
    const safe = settings || {};
    const appName = safe.appName || "CBT SMP";
    const schoolName = safe.schoolName || "Sekolah";
    document.title = `${appName} · ${schoolName}`;
    if (headerBrand) headerBrand.textContent = appName;
    const manifestTheme = document.querySelector('meta[name="description"]');
    if (manifestTheme) manifestTheme.setAttribute("content", `${appName} ${schoolName} dengan database terpusat.`);
  }

  function periodText(settings = state.settings) {
    return [settings?.academicYear, settings?.semester].filter(Boolean).join(" · ");
  }

  async function loadPublicInfo() {
    publicInfo = await rpc("app_public_info");
    if (publicInfo?.settings) {
      state.settings = { ...state.settings, ...publicInfo.settings };
  ...
  function renderConfiguration() {
    ...
    document.getElementById("configForm").addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (publicInfo?.initialized) renderLogin();
        else renderInitialSetup();
      } catch (error) {
  ...
  function renderInitialSetup() {
    ...
    app.innerHTML = `
      <section class="auth-page">
        <div class="auth-card card setup-card">
          <div class="hero-mark">1×</div>
          <h1>Konfigurasi pertama</h1>
          <p class="muted">Buat identitas sekolah dan akun administrator. Formulir ini hanya dapat digunakan sekali.</p>
          <form id="initialSetupForm" class="stack">
            <div class="form-grid">
              <label>Nama sekolah<input name="schoolName" required value="${h(s.schoolName || "")}" placeholder="Contoh: SMP Nusantara"></label>
              <label>Nama aplikasi<input name="appName" required value="${h(s.appName || "CBT SMP")}" placeholder="CBT SMP"></label>
              <label>Tahun ajaran<input name="academicYear" value="${h(s.academicYear || "")}" placeholder="2026/2027"></label>
              <label>Semester
            ...
            <label>Nama lengkap<input name="adminName" required placeholder="Nama guru/administrator"></label>
            <label>Username<input name="username" required minlength="3" maxlength="40" pattern="[a-z0-9._-]+" placeholder="admin.sekolah" autocapitalize="none"></label>
            <label>Kata sandi
              <div class="password-wrap">
                <input id="setupPassword" name="password" type="password" required minlength="8" autocomplete="new-password" placeholder="Minimal 8 karakter">
                <button id="toggleSetupPassword" type="button" class="icon-button" aria-label="Tampilkan kata sandi">👁</button>
              </div>
            </label>
            <label>Ulangi kata sandi<input name="passwordConfirm" type="password" required minlength="8" autocomplete="new-password"></label>
            <label class="checkbox-line"><input name="acknowledge" type="checkbox" required> Saya sudah menyimpan username dan kata sandi administrator.</label>
            <button class="btn btn-primary btn-block" type="submit">Buat sistem CBT</button>
          </form>
          <div class="card setup-note"><strong>Catatan keamanan</strong><p class="muted">Gunakan kata sandi unik. Jangan membagikan akun administrator kepada siswa.</p></div>
        </div>
      </section>`;

    const form = document.getElementById("initialSetupForm");
    const password = document.getElementById("setupPassword");
    document.getElementById("toggleSetupPassword").addEventListener("click", (event) => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      if (pass !== String(data.get("passwordConfirm") || "")) {
        return toast("Konfirmasi kata sandi tidak sama.", "error");
      }
      setButtonBusy(button, true, "Membuat sistem…");
      try {
        const result = await rpc("app_initial_setup", {
          p_school_name: String(data.get("schoolName") || "").trim(),
          p_app_name: String(data.get("appName") || "").trim(),
          p_academic_year: String(data.get("academicYear") || "").trim(),
          p_semester: String(data.get("semester") || "").trim(),
          p_admin_name: String(data.get("adminName") || "").trim(),
          p_username: String(data.get("username") || "").trim().toLowerCase(),
          p_password: pass
        });
        state.token = result.token;
        setStored(SESSION_KEY, { token: state.token });
        await loadBootstrap();
        publicInfo = { initialized: true, settings: state.settings };
        toast("Sistem berhasil dibuat. Anda masuk sebagai administrator.");
        routeUser();
      } catch (error) {
        toast(friendlyError(error), "error");
      } finally {
  ...
  function renderLogin() {
    ...
    document.getElementById("loginAppName").textContent = state.settings.appName || "CBT SMP";
    document.getElementById("loginSchoolName").textContent = state.settings.schoolName || "Masuk sebagai guru atau siswa.";
    document.getElementById("loginPeriod").textContent = periodText(state.settings);
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        const data = await rpc("app_login", {
          p_username: username.value.trim().toLowerCase(),
  ...
  function renderTeacherTab() {
    ...
    if (teacherTab === "settings") renderSettingsManagement(content);
  ...
  function renderStudentManagement(content) {
    ...
    content.innerHTML = `
      <section class="section-block">
        <div class="section-heading">
          <div><p class="eyebrow">Peserta terpusat</p><h2>Data Siswa</h2></div>
          <div class="toolbar-group">
            <button id="downloadStudentTemplateBtn" class="btn btn-ghost btn-small">Template CSV</button>
            <button id="importStudentsBtn" class="btn btn-secondary btn-small">Impor CSV</button>
            <button id="addStudentBtn" class="btn btn-primary">+ Tambah siswa</button>
          </div>
        </div>
        ...
        </tbody></table></div>` : `<div class="empty-state">Belum ada siswa. Tambahkan satu per satu atau impor melalui CSV.</div>`}
      </section>`;
    ...
    document.getElementById("importStudentsBtn").addEventListener("click", openStudentImport);
  ...
  function downloadStudentTemplate() {
    const csv = "\ufeffusername,password,name,className\nsiswa001,SandiAman01,Nama Siswa,VIII A\nsiswa002,SandiAman02,Nama Siswa,VIII A";
    downloadBlob(csv, "template-impor-siswa.csv", "text/csv;charset=utf-8");
  }
  ...
  function openStudentImport() {
    showModal(`
      ...
      <p class="muted">Gunakan kolom <code>username,password,name,className</code>. Username yang sudah ada akan diperbarui. Kata sandi kosong pada akun lama tidak akan mengubah kata sandinya.</p>
      <form id="studentImportForm" class="stack">
        <label>Pilih file CSV<input id="studentCsvFile" type="file" accept=".csv,text/csv" required></label>
        <div id="studentImportPreview" class="card setup-note"><strong>Belum ada file dipilih.</strong></div>
        <div class="modal-footer">
          <button type="button" id="cancelStudentImport" class="btn btn-secondary">Batal</button>
          <button type="submit" class="btn btn-primary" disabled>Impor siswa</button>
        </div>
      </form>`, () => {
      let parsedStudents = [];
      ...
      document.getElementById("cancelStudentImport").addEventListener("click", closeModal);
      ...
      fileInput.addEventListener("change", async () => {
        ...
        try {
          ...
          if (rows.length < 2) throw new Error("CSV tidak memiliki data siswa.");
          const headers = rows[0].map((cell) => cell.trim());
          const required = ["username", "password", "name", "className"];
          const indexMap = Object.fromEntries(required.map((key) => [key, headers.indexOf(key)]));
          if (required.some((key) => indexMap[key] < 0)) {
            throw new Error("Header harus persis: username,password,name,className");
          }
          parsedStudents = rows.slice(1).map((cells) => ({
            username: String(cells[indexMap.username] || "").trim().toLowerCase(),
            password: String(cells[indexMap.password] || ""),
            name: String(cells[indexMap.name] || "").trim(),
            className: String(cells[indexMap.className] || "").trim()
          })).filter((item) => item.username || item.name);
          if (!parsedStudents.length) throw new Error("Tidak ada baris siswa yang dapat diimpor.");
          if (parsedStudents.length > 1000) throw new Error("Maksimal 1.000 siswa per impor.");
          preview.innerHTML = `<strong>${parsedStudents.length} siswa siap diimpor</strong><p class="muted tiny">Contoh: ${h(parsedStudents[0].username)} · ${h(parsedStudents[0].name)} · ${h(parsedStudents[0].className || "-")}</p>`;
          submit.disabled = false;
        } catch (error) {
          preview.innerHTML = `<strong>File tidak valid</strong><p class="muted tiny">${h(friendlyError(error))}</p>`;
        }
      });
      ...
      form.addEventListener("submit", async (event) => {
        ...
        setButtonBusy(button, true, "Mengimpor…");
        try {
          const result = await rpc("app_bulk_save_students", { p_token: state.token, p_students: parsedStudents }, { timeout: 60000 });
          const summary = `${result.created || 0} dibuat, ${result.updated || 0} diperbarui, ${result.failed || 0} gagal.`;
          closeModal();
          toast(summary, result.failed ? "error" : "");
          await refreshAndRoute();
        } catch (error) {
          toast(friendlyError(error), "error");
          setButtonBusy(button, false);
  ...
  function openStudentEditor(studentId = null) {
    ...
    showModal(`
      ...
      <form id="studentEditorForm" class="stack">
        ...
        <label>${student ? "Kata sandi baru (opsional)" : "Kata sandi"}<input name="password" ${student ? "" : "required"} type="password" placeholder="Minimal 8 karakter"></label>
      ...
      document.getElementById("studentEditorForm").addEventListener("submit", async (event) => {
        ...
        if (password && password.length < 8) return toast("Kata sandi minimal 8 karakter.", "error");
        const record = {
          id: studentId || null,
  ...
  async function clearResults(event) {
    const confirmation = prompt("Tindakan ini menghapus seluruh hasil ujian. Ketik HAPUS HASIL untuk melanjutkan.");
    if (confirmation !== "HAPUS HASIL") return;
  ...
  function renderSettingsManagement(content) {
    const s = state.settings || {};
    content.innerHTML = `
      <section class="section-block">
        <div class="section-heading"><div><p class="eyebrow">Identitas dan tampilan</p><h2>Pengaturan Sekolah</h2></div></div>
        <form id="schoolSettingsForm" class="card settings-card stack">
          <div class="form-grid">
            <label>Nama sekolah<input name="schoolName" required value="${h(s.schoolName || "")}" placeholder="Nama sekolah"></label>
            <label>Nama aplikasi<input name="appName" required value="${h(s.appName || "CBT SMP")}" placeholder="CBT SMP"></label>
            <label>Tahun ajaran<input name="academicYear" value="${h(s.academicYear || "")}" placeholder="2026/2027"></label>
            <label>Semester
              <select name="semester">
                <option value="" ${!s.semester ? "selected" : ""}>Tidak ditampilkan</option>
          ...
          <div class="settings-options">
            <label class="checkbox-line"><input name="showStudentScore" type="checkbox" ${s.showStudentScore !== false ? "checked" : ""}> Tampilkan nilai dan jumlah benar kepada siswa setelah ujian</label>
            <label class="checkbox-line"><input name="showStudentHistory" type="checkbox" ${s.showStudentHistory !== false ? "checked" : ""}> Tampilkan riwayat ujian pada dashboard siswa</label>
          </div>
          <div class="toolbar"><p class="tiny muted">Perubahan nama aplikasi dan sekolah langsung terlihat pada semua perangkat setelah memuat ulang.</p><button class="btn btn-primary" type="submit">Simpan pengaturan</button></div>
        </form>
      </section>`;

    document.getElementById("schoolSettingsForm").addEventListener("submit", async (event) => {
      event.preventDefault();
      const button = event.submitter;
      const data = new FormData(event.currentTarget);
      const payload = {
        ...
        showStudentScore: data.has("showStudentScore"),
        showStudentHistory: data.has("showStudentHistory")
      };
      setButtonBusy(button, true);
      try {
        state.settings = await rpc("app_save_settings", { p_token: state.token, p_settings: payload });
        applyBranding(state.settings);
  ...
  function openChangePassword() {
    ...
    showModal(`
      ...
      <p class="muted">Akun: <strong>${h(state.user.username)}</strong></p>
      <form id="changePasswordForm" class="stack">
        ...
        <label>Kata sandi baru<input name="newPassword" type="password" required minlength="8" autocomplete="new-password" placeholder="Minimal 8 karakter"></label>
        <label>Ulangi kata sandi baru<input name="confirmPassword" type="password" required minlength="8" autocomplete="new-password"></label>
        <div class="modal-footer"><button id="cancelPasswordChange" type="button" class="btn btn-secondary">Batal</button><button type="submit" class="btn btn-primary">Ubah kata sandi</button></div>
      </form>`, () => {
      ...
      document.getElementById("changePasswordForm").addEventListener("submit", async (event) => {
        ...
        if (newPassword !== String(data.get("confirmPassword") || "")) {
          return toast("Konfirmasi kata sandi baru tidak sama.", "error");
        }
        setButtonBusy(button, true, "Mengubah…");
        try {
          await rpc("app_change_password", {
            p_token: state.token,
  ...
  function renderStudent() {
    ...
    const results = [...state.results].sort((a, b) => new Date(b.submittedAt) - new Date(a.submittedAt));
    const scoreVisible = state.settings.showStudentScore !== false;
    const historyVisible = state.settings.showStudentHistory !== false;
    const scoredResults = results.filter((result) => result.scoreVisible !== false && Number.isFinite(Number(result.score)));
    const best = scoredResults.length ? Math.max(...scoredResults.map((result) => Number(result.score))) : "-";
    ...
    document.getElementById("studentStats").innerHTML = [
      ...
      scoreVisible ? statCard("Nilai terbaik", best) : statCard("Nilai", "Disembunyikan"),
    ...
    examList.innerHTML = exams.length ? exams.map((exam) => {
      ...
      const lastNote = lastAttempt
        ? (lastAttempt.scoreVisible === false
          ? `<p class="tiny muted">Ujian sudah dikumpulkan.</p>`
          : `<p class="tiny muted">Percobaan terakhir: nilai ${lastAttempt.score}</p>`)
        : "";
    ...
    const historySection = document.getElementById("studentHistorySection");
    if (!historyVisible) {
    ...
    if (!results.length) {
      resultList.innerHTML = `<div class="empty-state">Belum ada riwayat ujian.</div>`;
    } else if (!scoreVisible) {
      resultList.innerHTML = `<div class="table-wrap"><table><thead><tr><th>Waktu</th><th>Ujian</th><th>Status</th></tr></thead><tbody>${results.map((result) => `<tr><td>${h(formatDate(result.submittedAt))}</td><td>${h(result.examTitle)}</td><td><span class="badge success">Terkumpul</span></td></tr>`).join("")}</tbody></table></div>`;
    } else {
      resultList.innerHTML = `<div class="table-wrap"><table><thead><tr><th>Waktu</th><th>Ujian</th><th>Benar</th><th>Nilai</th></tr></thead><tbody>${results.map((result) => `<tr><td>${h(formatDate(result.submittedAt))}</td><td>${h(result.examTitle)}</td><td>${result.correctCount}/${result.totalQuestions}</td><td>${scoreBadge(Number(result.score), Number(result.passingScore))}</td></tr>`).join("")}</tbody></table></div>`;
    }
  }
  ...
  function showExamResult(result, timeExpired = false) {
    ...
    const passed = scoreVisible && Number(result.score) >= Number(result.passingScore);
    clearTimer();
    clearAttemptBackup();
    examRuntime = null;
    submittingExam = false;
    showModal(scoreVisible ? `
      ...
      <div class="modal-footer"><button id="backToDashboardBtn" class="btn btn-primary btn-block" type="button">Kembali ke dashboard</button></div>` : `
      <div class="center"><p class="eyebrow">Ujian selesai</p><h2>${h(result.examTitle)}</h2><div class="result-score">✓</div><h3>Jawaban berhasil dikumpulkan.</h3><p class="muted">${timeExpired || result.timeExpired ? "Waktu ujian telah habis. " : ""}Nilai tidak ditampilkan kepada siswa sesuai pengaturan sekolah.</p><p class="tiny muted">Hasil telah tersimpan di database guru.</p></div>
      ...
      closeModal();
      try { await refreshAndRoute("Memuat data terbaru…"); }
  ...
  (async () => {
    if (!getConfig().configured) return renderConfiguration();
    renderLoading("Menghubungkan ke database…");
    try {
      await loadPublicInfo();
      if (!publicInfo?.initialized) return renderInitialSetup();
